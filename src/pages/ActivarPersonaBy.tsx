...
const ActivarPersonaBy: React.FC = () => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const activarCedula = async (cedula: string) => {
    try {
      ...
      const res = await RegistrosService.activarParticipanteByMunicipio(
        cedula,
        municipio
      );
      ...
      <div className="w-full md:max-w-xl bg-white rounded-xl shadow-xl overflow-hidden">
        <div className="p-6">
          ...
          <h1 className="text-2xl text-center uppercase font-black mb-4 text-green-900">
            ACTIVAR PARTICIPANTE de{" "}
            <span className="text-orange-600">{municipio}</span>
          </h1>
...
export default ActivarPersonaBy;
